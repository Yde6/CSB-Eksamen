...
def adaptivestep(f,xL,xR,tol):
    T2 = trapezoidalrule(f,xL,xR,1)
    T4 = trapezoidalrule(f,xL,xR,2)
    xM = (xL+xR)/2
    quadpoints.append(xM) # bruges kun til plot
    if abs(T4-T2) <= 15*tol: # bemærk at faktoren 15 er specifik for Simpons regel
        quadpoints.append((xL+xM)/2) # bruges kun til plot
        quadpoints.append((xM+xR)/2) # bruges kun til plot
        return T4 # denne approksimation har fejl ca. tol/(2^k) på delintervallet
    else:
